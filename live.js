...
function render(m){
 ...
 const resultBanner = m.status==='COMPLETED' && m.resultText ? `<section class="match-result-banner"><div class="result-kicker">🏆 MATCH RESULT</div><div class="result-main">${esc(m.resultText)}</div><div class="result-sub">${esc(m.winnerTeamName||'')} ${m.winnerTeamName?'— বিজয়ী দল':''}</div></section>` : m.status==='INNINGS_BREAK' ? `<section class="innings-break-banner"><div class="result-kicker">⏸️ INNINGS BREAK</div><div class="result-main">১ম Innings শেষ — ${esc(first?.battingTeam||m.battingTeam||'')} ${num(first?.score||m.score)}/${num(first?.wickets||m.wickets)}</div><div class="result-sub">২য় Innings শুরু হলে Live Score এখানে automatically update হবে।</div></section>` : m.status==='SUPER_OVER_READY' ? `<section class="match-result-banner tie"><div class="result-kicker">⚡ MATCH TIED</div><div class="result-main">Match tied — Super Over required</div><div class="result-sub">Super Over শুরু হলে Live Score automatically update হবে।</div></section>` : '';
 box.innerHTML=`${resultBanner}<section class="score-card"><div class="score-line"><div><span class="pill-live"><i class="dot"></i>${esc(m.status||'LIVE')}</span><div class="teams" style="margin-top:10px">${esc(m.battingTeam||m.team1Name||'')} <span class="muted2">vs</span> ${esc(m.bowlingTeam||m.team2Name||'')}</div><div class="muted2">${esc(m.season||'')} • ${esc(m.stageName||'Match')} • ${esc(m.id||'')}</div></div><div class="score">${num(m.score)}/${num(m.wickets)}</div></div>
