...
function printReceivedCopy(x){
  ...
  <body><div class="sheet"><h1>VIVEKANANDA CRICKET ASSOCIATION OF JELIAKHALI</h1><div class="sub">জেলিয়াখালী “মন্ডল এন্ড মন্ডল” ক্রিকেট টুর্নামেন্ট ২০২৬</div><div class="id">OFFICIAL PAYMENT RECEIPT<br>Registration ID: ${e(x.uniqueId)}<br><small>Receipt Verification Code: ${e(x.verificationCode||"-")}</small></div>
  ...
  <p><strong>Status:</strong> ${e(x.status)}</p><p><strong>Rules Accepted:</strong> ${e(x.rulesAccepted)} &nbsp; <strong>Stay Required:</strong> ${e(x.stayRequired)}</p><p><strong>Entry Fee:</strong> ${e(x.entryFee?.paymentMethod)} | ${e(x.entryFee?.paymentDate)} | Received by: ${e(x.entryFee?.receivedBy)}</p><p><strong>Kasanmani:</strong> ${e(x.kasanmani?.paymentMethod)} | ${e(x.kasanmani?.paymentDate)} | Received by: ${e(x.kasanmani?.receivedBy)}</p><p style="color:#475569;font-size:13px"><strong>PAYMENT RECEIPT / REGISTRATION RECEIPT</strong><br>Receipt ID ও Verification Code-এর মাধ্যমে এই receipt-এর তথ্য যাচাই করা যাবে। Receipt-এর তথ্য পরিবর্তন করলে verification-এর সঙ্গে মিলবে না।</p><button class="print" onclick="window.print()">Print / Save as PDF</button></div></body></html>`);
