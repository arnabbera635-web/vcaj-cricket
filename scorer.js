...
async function confirmWicket(){const batter=selectedBatter($("dismissedBatter").value);const type=$("dismissalType").value;const fielder=(state.bowlers||[]).find(x=>x.id===$("fielder").value);if(!batter){alert("আউট হওয়া ব্যাটার নির্বাচন করুন।");return;}if(type==="caught"&&!fielder){alert("ক্যাচের ফিল্ডার নির্বাচন করুন।");return;}closeWicket();await applyWicket(type,batter,fielder);}
async function applyWicket(type,batter, fielder){if(!state||state.status!=="LIVE")return;const before=JSON.parse(JSON.stringify(state));const bowler=(state.bowlers||[]).find(x=>x.id===state.bowler);if(!bowler){alert("আগে বোলার নির্বাচন করুন।");return;}state.wickets++;batter.out=true;bowler.wickets++;if(type!=="timedOut"&&type!=="retiredOut")batter.balls++;if(batter.id===state.striker){const replacement=(state.players||[]).find(p=>!p.out&&p.id!==state.nonStriker);state.striker=replacement?.id||"";}else if(batter.id===state.nonStriker){const replacement=(state.players||[]).find(p=>!p.out&&p.id!==state.striker);state.nonStriker=replacement?.id||"";}if(type!=="timedOut"&&type!=="retiredOut"){state.legalBalls++;bowler.balls++;}bowler.runs+=0;const text=buildDismissalText(type,batter,fielder);const ev={type:"wicket",dismissalType:type,dismissedBatterId:batter.id,dismissedBatter:batter.name,fielderId:fielder?.id||"",fielderName:fielder?.name||"",batterRuns:0,legal:type!=="timedOut"&&type!=="retiredOut",strikerId:batter.id,over:ballsText(before.legalBalls),text,createdAt:new Date().toISOString()};state.commentary.push(ev);eventStack.push(before);if(state.legalBalls>=Number(state.maxOvers||15)*6)state.status="INNINGS_BREAK";render();await addDoc(collection(db,"matches",state.id,"events"),ev);await save();const bref=doc(db,"players",bowler.id);const bsnap=await getDoc(bref);const bcur=bsnap.exists()?bsnap.data():{id:bowler.id,name:bowler.name,totalWickets:0,totalBalls:0,totalRuns:0};await setDoc(bref,{...bcur,id:bowler.id,name:bowler.name,totalWickets:(bcur.totalWickets||0)+1,totalBalls:(bcur.totalBalls||0)+(ev.legal?1:0)},{merge:true});}
