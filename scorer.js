...
async function loadMatch(id){id=(id||"").trim();if(!id)return;localStorage.setItem("vcajActiveMatchId",id);const snap=await getDoc(doc(db,"matches",id));if(!snap.exists()){alert("Match পাওয়া যায়নি। আগে Match Setup থেকে তৈরি করুন।");return;}state=snap.data();eventStack=[];render();if(unsub)unsub();unsub=onSnapshot(doc(db,"matches",id),s=>{if(s.exists()){state=s.data();render();}});}
